...
GoogleSignin.configure({
  webClientId:
    '896982317636-48kai8erdjk2umhpsa657tak2nce55qn.apps.googleusercontent.com',
});
...
function NavigationStack() {
  ...
  async function login() {
    ...
    try {
      ...
      const response = await api.post(
        'https://vq8w0bp7ee.execute-api.us-west-1.amazonaws.com/sign-in',
        requestBody,
        headers,
      );
      if (response.status) {
  ...
  async function fetchS3Details() {
    ...
    try {
      const response = await api.post(
        'https://u6mj6kk2h1.execute-api.us-west-1.amazonaws.com/findRegion',
        requestBody,
        headers,
      );
      if (response.status) {
